...
def make_data_file_from_json(data):
    ...
    for level_data in data: #look at all the level data
        #set the level
        level = CCLevel()
        level.level_number = level_data["LevelNumber"]
        #set the time limit
        level.time = level_data["Time"]
        #set the number of chips
        level.num_chips = level_data["ChipNumber"]
        #access the upper and lower layers in the json
        level.upper_layer = level_data["UpperLayer"]
        level.lower_layer = level_data["LowerLayer"]
...
default_input_json_file = "smsyjuco_ccl.json" # the json file we are accessing
